            ...
            success: function (response) {
                ...
                for (let i = 0; i < loginUserList.length; i++) {
                    if ((loginUserList[i].uname == $("#userNameId").val()) && loginUserList[i].pwd == $("#pwdId").val()) {
                        ...
                        $("#userNameId").val('');
                        window.location.href = "landBidding.html";
...
$(document).ready(function () {
    ...
    $("#userTypeId").change(function () {
        if ($(this).val() == 'consumer') {
            $("#landDetailsId").find('input:text,input[name="email"], input:password, input:file, select, textarea')
                .each(function () {
